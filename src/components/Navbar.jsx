...
const Navbar = () => {
  ...
  return (
    <nav className="bg-gray-800 text-white p-4 flex justify-between items-center" role="navigation" aria-label="Main Navigation">
      <div className="logo">
        <Link to="/" className="text-xl font-bold" aria-label="Go to homepage">Logo</Link>
      </div>
      ...
      <ul
        ref={menuRef}
        id="navbar-menu"
        className={`navbar-menu fixed top-16 left-0 bg-gray-800 flex flex-col items-start p-4 space-y-4 md:flex md:flex-row md:space-y-0 md:space-x-4 md:static md:h-auto md:bg-transparent ${menuOpen ? 'block' : 'hidden'}`}
      >
        ...
        >
          <Link to="#" className="hover:text-gray-400" aria-label="Open Get Started menu">Get Started</Link>
          <ul
            className={`absolute bg-gray-700 p-2 mt-1 transition-all duration-300 ease-in-out ${dropdownOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}
